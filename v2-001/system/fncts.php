...
function genPageNavbar($MOD, $tit=NULL, $des='h1',$icon=NULL,$css='navbar-fixed-top'){
	$banMod=FALSE;
	if($MOD){
		$rowMod=detMod($MOD);
...
$query_create_table = "CREATE TEMPORARY TABLE db_clientes_temp (id int(11) NOT NULL auto_increment, cod_pac int(6), fullname varchar(100), fulltext(fullname), PRIMARY KEY (id))ENGINE = MYISAM;";
if (mysql_query($query_create_table)){}
else {echo mysql_error();}
$query_datos_pac='SELECT pac_cod, pac_nom, pac_ape FROM db_clientes';
...
{	do 
	{	$nom = $row_RS_datos_pac['pac_nom'].' '.$row_RS_datos_pac['pac_ape'];
		$query_insert_temp='INSERT INTO db_clientes_temp (cod_pac, fullname) VALUES ("'.$row_RS_datos_pac['pac_cod'].'" ,"'.$nom.'")';
...
function fnc_gencad_search(){
  ...
  $numero=count($trozos);
  if ($numero==1) $cadbusca='SELECT * FROM db_clientes_temp where fullname LIKE "%'.$busqueda.'%" ORDER BY cod_pac DESC'; 
  if ($numero>1)$cadbusca='SELECT cod_pac, fullname, MATCH (fullname) AGAINST ("'.$busqueda.'") AS Score FROM db_clientes_temp WHERE MATCH (fullname) AGAINST ("'.$busqueda.'") ORDER BY Score DESC';
  return $cadbusca;
}
...
function fnc_cadsearch($busqueda){
	...
	if((isset($busqueda))&&($busqueda!="")){
		...
		$msg_sys.="TBL.".tableExists("db_clientes_temp")."//";
		if(tableExists("db_clientes_temp")==0){
			...
			if($busqueda!=$_SESSION['sBr']){
				...
				mysql_query('DROP TABLE db_clientes_temp');
...
function fnc_datacons($param1, $param2){
	$query_RS_datos = sprintf("SELECT * FROM db_consultas INNER JOIN db_clientes ON db_consultas.pac_cod=db_clientes.pac_cod WHERE db_consultas.con_num=%s AND db_consultas.pac_cod=%s", 
		SSQL($param1, "int"),
		SSQL($param2, "int"));
	$RS_datos = mysql_query($query_RS_datos) or die(mysql_error());
	$row_RS_datos = mysql_fetch_assoc($RS_datos);
...
function fnc_totpac(){
	$query_RS_datos = sprintf("SELECT * FROM db_clientes");$RS_datos = mysql_query($query_RS_datos) or die(mysql_error()); $row_RS_datos = mysql_fetch_assoc($RS_datos); $totalRows_RS_datos = mysql_num_rows($RS_datos); return ($totalRows_RS_datos); mysql_free_result($RS_datos);
}
