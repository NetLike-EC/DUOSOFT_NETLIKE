...
$insertGoTo=$_SESSION['urlp'];
	if((isset($_POST['form']))&&($_POST['form']=='form_mod')){
		if((isset($action))&&($action=='UPD')){
			$qry=sprintf('UPDATE tbl_types SET mod_ref=%s, typ_ref=%s, typ_nom=%s, typ_val=%s WHERE typ_cod=%s',			
			GetSQLValueString($_POST['form_mod'],'text'),
			GetSQLValueString($_POST['form_ref'],'text'),
			GetSQLValueString($_POST['form_nom'],'text'),
			GetSQLValueString($_POST['form_val'],'text'),
		...
		if((isset($action))&&($action=='INS')){
			$qry=sprintf('INSERT INTO tbl_types (mod_ref, typ_ref, typ_nom, typ_val, typ_stat) 
			VALUES (%s,%s,%s,%s,%s)',
			GetSQLValueString($_POST['form_mod'],'text'),
			GetSQLValueString($_POST['form_ref'],'text'),
			GetSQLValueString($_POST['form_nom'],'text'),
