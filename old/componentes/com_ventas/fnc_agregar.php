...
if($_SESSION['stat_proc']=='PROCESS'){
	...
	if($totProd>0){
		...
		foreach ($productos as $v){
			...
			$ProdCan=$v["can"];
			$ProdPre=$v["pre"];
			$ProdPre = number_format($ProdPre,5,'.','');
		...
		$aud=AUD('Generacion Factura Venta');
		//INS CABECERA FACTURA
		$detCic=fnc_numIniCic();
		$detCic_id=$detCic['id'];
		$numFac=fnc_numMaxFac();
		$qryInsVen=sprintf("INSERT INTO tbl_factura_ven (fac_num, faccic_id, ven_num, aud_id, fac_obs, fac_stat) VALUES (%s,%s,%s,%s,%s,%s)",
		GetSQLValueString($numFac,'text'),
		GetSQLValueString($detCic_id,'text'),
		GetSQLValueString($ven_num,'int'),
		GetSQLValueString($aud,'int'),
		GetSQLValueString('','int'),
		GetSQLValueString('1','int'));
		mysql_query($qryInsVen)or($LOG.=mysql_error());
		$ven_id=mysql_insert_id();
		...
		if(!(mysql_error())){
			...
			$GoTo=$RAIZc."com_ventas/venta_detail.php?id=".$ven_id;
			...
			$GoTo=$RAIZc.'com_ventas/form.php';
		}
	}else{
