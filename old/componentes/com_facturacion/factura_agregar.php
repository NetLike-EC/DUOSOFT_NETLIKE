...
if($_SESSION['stat_proc']=='PROCESS')
{
	...
	if(@mysql_query("INSERT INTO tbl_factura_cab (fac_num, fac_cic_id, fac_fec, fac_tip, cli_cod, emp_cod, fac_stat) VALUES ('$fac_num', '$fac_cic_id', '$fec_sys', '1', '$id_cli', '$id_emp', 'V')")or($LOG.=mysql_error()))//GRABAR CABECERA FACTURA
	{
		...
		foreach ($productos as $v)//GRABAR DETALLES FACTURA
		{
			...
			$detalles_add=actualizacion_inventario_venta($id_prod_det, $id_prod_can);
			$LOG.="Inventarios Actualizados [OK]<br />";
			//echo "PRODUCTO ID: ".$id_prod_det.'<br />';
			foreach ($detalles_add as $v)
			{
				$inv_id_det=$v["inv_id"];//Registro de Inventario del cual se va a descargar el producto
				$det_cant=$v["det_cant"]; //Cantidad que se va a descargar del Inventario seleccionado anteriormente
				//echo "Inventario: '$inv_id_det' - Cantidad: '$det_cant' <br />";
				@mysql_query("INSERT INTO tbl_factura_det (fac_num, fac_cic_id, prod_cod, inv_id, fac_prod_can, fac_prod_pre) VALUES ('$fac_num', '$fac_cic_id', '$id_prod_det', '$inv_id_det', '$det_cant', '$id_prod_pre')")or($LOG.=mysql_error());
			}
		...
		if($tip_pag=='1')//Pago de Contado
		{
			...
			{
				if(@mysql_query("INSERT INTO tbl_cta_x_cob (fac_num, fac_cic_id, fec_ini, cta_plazo, cta_valor, cta_abono, emp_cod, cta_stat) 
				VALUES ('$fac_num', '$fac_cic_id', '$fec_sys', '0', '$tot_fac', 0, '$id_emp', 'V')")or($LOG.=mysql_error()))
				...
				if($tip_pag=='3')//Pago Credito
				{
					...
					if(@mysql_query("INSERT INTO tbl_cta_x_cob (fac_num, fac_cic_id, fec_ini, cta_plazo, cta_valor, cta_abono, emp_cod, cta_stat) 
					VALUES ('$fac_num', '$fac_cic_id', '$fec_sys', '$dias_cred', '$tot_fac', 0, '$id_emp', 'V')")or($LOG.=mysql_error()))
						$LOG.="Creada Cuenta X Cobrar [OK]<br />";
				}
			}
		}
		...
		$qry_update_num_fac="UPDATE tbl_factura_ciclosf SET fac_cic_cont=fac_cic_cont+1 WHERE fac_cic_id=".$row_RS_fact_act['fac_cic_id'];
			if(@mysql_query("$qry_update_num_fac")or($LOG.=mysql_error()))
				$LOG.="Secuencia Facturacion Actualizada <br />";
