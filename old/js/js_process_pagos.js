...
$(document).ready(function() {
	...
	var id_pac = $("#pac_sel_pag").attr("value");//Campo Hidden con ID_Cliente
		...
		else
		{
			...
			if(tip_pag==1)
			{
				...
				else
				{
					...
					if (confirm("Confirmar Pago: "+valor+".  Del Paciente: "+id_pac))
					{
						load_det_pag(valor,id_pac);
	...
	function load_det_pag(valor_send,id_pac_send){
	Shadowbox.open({
			content:    web_pag_norm+'?valor='+valor_send+'&id_pac='+id_pac_send,
			player:     "iframe",
